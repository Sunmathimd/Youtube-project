...
st.write("This is my First youtube project!!!")
...
mycursor.execute("CREATE DATABASE IF NOT EXISTS myyoutube")
mycursor.execute("USE myyoutube")
...
mycursor.execute('''CREATE TABLE IF NOT EXISTS video_details(Channel_name VARCHAR(255),
                                              ...
                                              Comments TEXT,
                                              Favorite_count INT)''')
mycursor.execute('''CREATE TABLE IF NOT EXISTS comment_details(Comment_id VARCHAR(355),
                                                 Video_id VARCHAR(355),
                                                 Comment_text TEXT,
                                                 Comment_author VARCHAR(355),
                                                 Comment_posted_date TEXT,
                                                 Like_count INT,
                                                 Reply_count INT)''')

sql = '''INSERT INTO channel_details(channel_id,
                                     Channel_name,
                                     Subscribers,
                                     Views,
                                     Description,
...
val = tuple(youtube ['channel_details'][0].values())
mycursor.execute(sql,val)
mydb.commit()  

sql = '''INSERT INTO video_details(Channel_name,
                                   Channel_id,
                                   Video_id,
                                   Title,
                                   Tags,
                                   ...
                                   Likes,
                                   Comments,
                                   Favorite_count)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'''
for i in youtube ['video_details']:
     val = tuple(i.values())
mycursor.execute(sql,val)
mydb.commit() 

sql = '''INSERT INTO comment_details(Comment_id,
                                     Video_id,
                                     Comment_text,
                                     Comment_author,
                                     Comment_posted_date,
...
for j in youtube ['comment_details']:
    val = tuple(j.values())
    mycursor.execute(sql,val)
mydb.commit()
...
api_key = "AIzaSyCnkys0C-3BBRMR_0uOrtgCTf6aj9mLqQ4" 
